...
class GeneradorRolesPagoINSEVIG:
    def __init__(self, root):
        ...
        try:
            # Buscar el icono en la carpeta del programa
    ...
    def obtener_datos_bd(self, periodo):
        ...
        try:
            ...
            try:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT COUNT(*) FROM [insevig].[dbo].[RPHISTOR] WITH (NOLOCK) "
                    "WHERE [FECHA_VEN] >= ? AND [FECHA_VEN] <= ?",
            ...
            df_dbtablas_fnc = pd.read_sql("SELECT CODIGO, NOMBRE FROM dbo.DBTABLAS WHERE TIPO = 'FNC'", conn)
            df_dbtablas_dpt = pd.read_sql("SELECT CODIGO, NOMBRE FROM dbo.DBTABLAS WHERE TIPO = 'DPT'", conn)
            df_dbtablas_sec = pd.read_sql("SELECT CODIGO, NOMBRE FROM dbo.DBTABLAS WHERE TIPO = 'SEC'", conn)
